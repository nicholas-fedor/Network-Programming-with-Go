...
func TestError(t *testing.T) {
	...
	if e1.Error != e2.Error {
		t.Errorf("expected error code %d; actual error code %d", e1.Error, e2.Error)
	}

	if e1.Message != e2.Message {
		t.Errorf("expected message %q; actual message %q", e1.Message, e2.Message)
	}
}
...
func TestReadReq(t *testing.T) {
	...
	if r1.Filename != r2.Filename {
		t.Errorf("expected filename %q; actual filename %q", r1.Filename, r2.Filename)
	}
